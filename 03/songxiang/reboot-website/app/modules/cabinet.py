...
def get(**kwargs):
    ...
    data = db.session.query(Cabinet).filter_by(**where).order_by \
        (getattr(getattr(Cabinet, order_by_list[0]), order_by_list[1])()).limit(limit).all()
    db.session.close()
...
def update(**kwargs):
    ...
    where = kwargs.get("where", {})
    check_update(Cabinet, data, where)
