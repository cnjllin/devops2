...
class AutoLoad():
    ...
    def isValidMethod(self, func):
        ...
        """
        self.func = func
        current_app.logger.debug("验证模块 {} 下的函数是否存在 {} 属性".format(self.module_name, self.func))
...
class JsonRpc():
    ...
    def validate(self):
        ...
        if self.jsonData.get("jsonrpc") != "2.0":
            ...
            current_app.logger.warning("jsonrpc版本信息不对, 应该为{}".format(self.VERSION))
            return False
    ...
    def callMethod(self, module, func, params, auth):
        ...
        if not at.isValidMethod(func_name):
            current_app.logger.warning("{} 模块下没有{} 这个方法".format(module_name, func_name))
        ...
        if self.requireAuthentication(module_name, func_name):
            # 需要登录/需要验证
            if auth is None:
                current_app.logger.warning("{}.{} 这个操作需要提供auth".format(module_name, func_name))
        ...
        try:
            ...
            if callable(called):
                ...
                current_app.logger.warning("{}下的{}方法不能被执行".format(module_name, func_name))
