...
def get(**kwargs):
    ...
    data = db.session.query(ManagementCard).filter_by(**where).order_by \
        (getattr(getattr(ManagementCard, order_by_list[0]), order_by_list[1])()).limit(limit).all()
    db.session.close()
...
def update(**kwargs):
    ...
    where = kwargs.get("where", {})
    check_update(ManagementCard, data, where)
