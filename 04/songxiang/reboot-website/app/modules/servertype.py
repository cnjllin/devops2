...
def get(**kwargs):
    ...
    data = db.session.query(ServerType).filter_by(**where).order_by \
        (getattr(getattr(ServerType, order_by_list[0]), order_by_list[1])()).limit(limit).all()
    db.session.close()
...
def update(**kwargs):
    ...
    where = kwargs.get("where", {})
    check_update(ServerType, data, where)
