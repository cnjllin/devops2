...
def get(**kwargs):
    ...
    data = db.session.query(Status).filter_by(**where).order_by \
        (getattr(getattr(Status, order_by_list[0]), order_by_list[1])()).limit(limit).all()
    db.session.close()
...
def update(**kwargs):
    ...
    where = kwargs.get("where", {})
    check_update(Status, data, where)
